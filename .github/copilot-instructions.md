...
## Project Overview
PDFSpliter is a modular, Android-first suite for advanced PDF processing, focused on Brazilian fiscal documents. Each tool (Dividir, UnirPDF, etc.) is fully isolated in its own directory with no shared code or utilities. All processing is 100% local (no server upload), and i18n is supported (Portuguese primary).

## Architecture & Patterns
- **Per-tool isolation:** Each tool has its own `index.html`, `script.js`, `style.css`, and Google verification file. Utility functions are duplicated per tool—never centralize utilities or share code between tools.
...
- **PDFtoArcgis:** Use multi-strategy coordinate extraction (`parseVertices()`, `detectDocIdFromPageText()`, etc. in [PDFtoArcgis/script.js](PDFtoArcgis/script.js)).
